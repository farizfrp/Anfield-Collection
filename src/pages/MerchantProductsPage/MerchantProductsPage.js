...
export default class MerchantProductsPage extends Component {
    state = {
...
PRODUCT=[];
   async  componentDidMount() {
    // const value =JSON.parse(await AsyncStorage.getItem('auth')) ;
    // console.log("merchantId",value)
    //   this.merchantId = value.auth.id;
    this.backHandler = BackHandler.addEventListener('hardwareBackPress', () => {
      Actions.MerchantMenuPage(); // works best when the goBack is async
      return true;
    });
...
renderItem={({item,index}) => (  <Swipeout right={ [
   ...
   <View style={{ borderBottomWidth: 0.3, color: "grey", paddingTop: 15 }}>
